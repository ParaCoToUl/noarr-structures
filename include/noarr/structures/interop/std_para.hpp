...
inline void std_for_each(const Traverser &t, const F &f) {
	std::for_each(std::execution::par, t.begin(), t.end(), [&f](const auto &t_inner) {
		t_inner.for_each(f);
	});
}
...
constexpr planner_std_execute_t planner_std_execute() noexcept {
	return planner_std_execute_t();
}

template<IsPlanner Planner>
inline void operator|(const Planner &planner, planner_std_execute_t) {
	std::for_each(std::execution::par, planner.begin(), planner.end(), [](const auto &inner_planner) {
