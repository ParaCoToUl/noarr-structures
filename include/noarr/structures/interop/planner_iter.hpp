...
public:
	[[deprecated("The default iterator for planners is not well-definable")]]
	explicit constexpr planner_iterator_t() noexcept
	...
	constexpr value_type operator*() const noexcept {
		return value_type(get_union(), get_order() ^ fix<Dim>(idx), get_ending());
	}

	[[nodiscard]]
	constexpr value_type operator[](difference_type i) const noexcept {
		return value_type(get_union(), get_order() ^ fix<Dim>(idx + i), get_ending());
	}
...
struct planner_range_t<Dim, union_t<Structs...>, Order, Ending> : flexible_contain<union_t<Structs...>, Order, Ending> {
	...
	constexpr value_type operator[](size_type i) const noexcept {
		return value_type(get_union(), get_order() ^ fix<Dim>(begin_idx + i), get_ending());
	}
};
