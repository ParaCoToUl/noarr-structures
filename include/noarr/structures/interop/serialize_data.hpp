...
constexpr decltype(auto) deserialize_data(Istream &&in, Struct s, void *data) noexcept {
	traverser(s).for_each([&in, s, data]<IsState State>(State state) noexcept { in >> (s | get_at(data, state)); });
...
constexpr decltype(auto) serialize_data(Ostream &&out, Struct s, const void *data) noexcept {
	traverser(s).for_each(
		[&out, s, data]<IsState State>(State state) noexcept { out << (s | get_at(data, state)) << '\n'; });
