...
template<class T, auto... Dims, IsState State> requires IsDimPack<decltype(Dims)...>
...
private:
	...
	static constexpr bool get_value() noexcept {
		if constexpr (structure::template has_length<DimMajor, indexless_state>() && structure::template has_length<DimMinor, indexless_state>()) {
			if constexpr (State::template contains<index_in<DimMinor>> && !State::template contains<index_in<DimMajor>>) {
				return false;
			} else {
				return is_contiguous<T, typename structure::template sub_state_t<State>>::value;
			}
...
template<IsDim auto Dim, IsDim auto DimIsBorder, IsDim auto DimMajor, IsDim auto DimMinor, class T, class MinorLenT, IsState State>
...
private:
	...
	static constexpr bool get_value() noexcept {
		if constexpr (State::template contains<length_in<DimMajor>, length_in<DimMinor>, index_in<DimMajor>, index_in<DimMinor>, index_in<DimIsBorder>>) {
			// TODO:
		} else {
			return false;
		}
...
// contiguous tuple
static_assert(is_contiguous<tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, state<>>::value);

static_assert(is_contiguous<tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, state<state_item<index_in<'t'>, lit_t<0>>>>::value);
static_assert(!is_contiguous<tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, state<state_item<index_in<'t'>, lit_t<1>>>>::value);
...
// contiguous fix
static_assert(is_contiguous<fix_t<'t', tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, lit_t<0>>, state<>>::value);
static_assert(!is_contiguous<fix_t<'t', tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, lit_t<1>>, state<>>::value);

// contiguous set_length
static_assert(is_contiguous<set_length_t<'x', vector_t<'x', scalar<int>>, std::size_t>, state<>>::value);
static_assert(is_contiguous<set_length_t<'x', tuple_t<'t', scalar<int>, vector_t<'x', scalar<int>>>, std::size_t>, state<>>::value);

// contiguous reorder
static_assert(is_contiguous<reorder_t<vector_t<'y', vector_t<'x', scalar<int>>>, 'x'>, state<state_item<length_in<'x'>, std::size_t>, state_item<length_in<'y'>, std::size_t>>>::value);
static_assert(!is_contiguous<reorder_t<vector_t<'y', vector_t<'x', scalar<int>>>, 'x'>, state<>>::value);

// contiguous hoist
static_assert(is_contiguous<hoist_t<'x', vector_t<'x', scalar<int>>>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<hoist_t<'x', vector_t<'x', scalar<int>>>, state<>>::value);

// contiguous rename
static_assert(is_contiguous<rename_t<vector_t<'x', scalar<int>>, 'x', 'y'>, state<state_item<length_in<'y'>, std::size_t>>>::value);
static_assert(!is_contiguous<rename_t<vector_t<'x', scalar<int>>, 'x', 'y'>, state<>>::value);

// contiguous join
static_assert(is_contiguous<join_t<vector_t<'y', vector_t<'x', scalar<int>>>, 'x', 'y', 'z'>, state<state_item<length_in<'z'>, std::size_t>>>::value);
static_assert(!is_contiguous<join_t<vector_t<'y', vector_t<'x', scalar<int>>>, 'x', 'y', 'z'>, state<>>::value);

// contiguous shift
static_assert(is_contiguous<shift_t<'x', vector_t<'x', scalar<int>>, std::size_t>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<shift_t<'x', vector_t<'x', scalar<int>>, std::size_t>, state<>>::value);

// contiguous slice
static_assert(is_contiguous<slice_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<slice_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<>>::value);

// contiguous span
static_assert(is_contiguous<span_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<span_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<>>::value);

// contiguous step
static_assert(is_contiguous<step_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<state_item<length_in<'x'>, std::size_t>, state_item<index_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<step_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<step_t<'x', vector_t<'x', scalar<int>>, std::size_t, std::size_t>, state<>>::value);

// contiguous reverse
static_assert(is_contiguous<reverse_t<'x', vector_t<'x', scalar<int>>>, state<state_item<length_in<'x'>, std::size_t>>>::value);
static_assert(!is_contiguous<reverse_t<'x', vector_t<'x', scalar<int>>>, state<>>::value);

// contiguous into_blocks
static_assert(is_contiguous<into_blocks_t<'x', 'y', 'z', vector_t<'x', scalar<int>>>, state<state_item<length_in<'y'>, std::size_t>, state_item<length_in<'z'>, std::size_t>>>::value);
static_assert(is_contiguous<into_blocks_t<'x', 'y', 'z', vector_t<'x', scalar<int>>>, state<state_item<length_in<'y'>, std::size_t>, state_item<length_in<'z'>, std::size_t>, state_item<index_in<'y'>, std::size_t>>>::value);
static_assert(!is_contiguous<into_blocks_t<'x', 'y', 'z', vector_t<'x', scalar<int>>>, state<state_item<length_in<'y'>, std::size_t>, state_item<length_in<'z'>, std::size_t>, state_item<index_in<'z'>, std::size_t>>>::value);
...
constexpr auto is_contiguous(State /*unused*/) noexcept {
	return []<class Struct>(Struct /*unused*/) constexpr noexcept {
		return is_contiguous<Struct, State>();
	};
}
