...
struct planner_endings : flexible_contain<Endings...> {
	...
	requires helpers::is_activated_v<decltype(this -> order(order))>
	{
	...
	[[nodiscard]]
	static constexpr std::size_t activated_index() noexcept
	requires activated
	{
	...
	template<std::size_t... Is>
	[[nodiscard]]
	static constexpr std::size_t activated_index(std::index_sequence<Is...> /*is*/) noexcept
	requires activated
	{
		return (0 + ... +
		        (helpers::is_activated_v<decltype(std::declval<planner_endings>().template get<Is>())> ? Is : 0));
	}
	...
	template<class Planner>
	constexpr void operator()(const Planner &planner) const
	requires helpers::is_activated_v<
		decltype(this->order(fix(state_at<typename Planner::union_struct>(planner.top_struct(), empty_state))))>
	{
		using transformed =
			decltype(order(fix(state_at<typename Planner::union_struct>(planner.top_struct(), empty_state))));
...
struct planner_t<union_t<Structs...>, Order, Ending> : flexible_contain<union_t<Structs...>, Order, Ending> {
	...
	[[nodiscard]]
	constexpr union_struct get_union() const noexcept {
		return base::template get<0>();
	}

	[[nodiscard]]
	constexpr Order get_order() const noexcept {
		return base::template get<1>();
	}

	[[nodiscard]]
	constexpr Ending get_ending() const noexcept {
	...
	constexpr auto pop_endings() const noexcept {
		return make_planner(get_union(), get_order(),
		                    get_ending().get_next(fix(state_at<union_struct>(top_struct(), empty_state))));
	}
...
private:
	...
	constexpr void for_each_impl(dim_sequence<> /*ds*/, State state) const {
		static_assert(
			helpers::is_activated_v<decltype(get_ending().order(fix(state_at<union_struct>(top_struct(), state))))>);
		get_ending()(order(fix(state)));
	}
};

template<class Union, class Order, class Ending>
planner_t(Union &&, Order &&, Ending &&)
	-> planner_t<std::remove_cvref_t<Union>, std::remove_cvref_t<Order>, std::remove_cvref_t<Ending>>;
...
constexpr auto operator^(const P &p, const helpers::for_sections_t<F, Dims...> &f)
	-> decltype(p.template for_sections<Dims...>(f)) {
	return p.template for_sections<Dims...>(f);
}

template<IsPlanner P, auto... Dims, class F>
requires (sizeof...(Dims) > 0)
constexpr auto operator^(const P &p, const helpers::for_dims_t<F, Dims...> &f)
	-> decltype(p.template for_sections<Dims...>(f).order(hoist<Dims...>())) {
...
template<IsPlanner P>
constexpr void operator|(const P &p, planner_execute_t /*exec*/) {
	p.execute();
}
