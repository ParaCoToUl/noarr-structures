...
template<auto QDim, IsState State, auto Dim, class ArgLength, class RetSig> requires (Dim != QDim) && IsDim<decltype(Dim)> && IsDim<decltype(QDim)>
...
template<auto QDim, IsState State, class ArgLength, class RetSig> requires IsDim<decltype(QDim)>
struct sig_find_dim_impl<QDim, State, function_sig<QDim, ArgLength, RetSig>> {
	using type = function_sig<QDim, ArgLength, RetSig>;
};
template<auto QDim, IsState State, auto Dim, class ...RetSigs> requires (Dim != QDim) && IsDim<decltype(Dim)> && IsDim<decltype(QDim)>
...
template<auto QDim, IsState State, class ...RetSigs> requires IsDim<decltype(QDim)>
struct sig_find_dim_impl<QDim, State, dep_function_sig<QDim, RetSigs...>> {
	using type = dep_function_sig<QDim, RetSigs...>;
};
template<auto QDim, IsState State, class ValueType> requires IsDim<decltype(QDim)>
