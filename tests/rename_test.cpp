...
TEST_CASE("Simple rename use-case", "[rename]") {
	...
	STATIC_REQUIRE(std::is_same_v<typename decltype(structure_renamed)::signature, typename array_t<'y', 10, array_t<'x', 20, scalar<int>>>::signature>);
...
TEST_CASE("Rename with reorder", "[rename]") {
	...
	STATIC_REQUIRE(std::is_same_v<typename decltype(structure_renamed)::signature, typename array_t<'x', 20, array_t<'y', 10, scalar<int>>>::signature>);
	REQUIRE((structure | offset<'x', 'y'>(3, 5)) == (structure_renamed | offset<'y', 'x'>(3, 5)));
}
...
TEST_CASE("Rename in tuple", "[rename]") {
	...
	STATIC_REQUIRE(std::is_same_v<typename decltype(structure_renamed)::signature, typename tuple_t<'t', array_t<'y', 10, array_t<'x', 20, scalar<int>>>, array_t<'y', 10, array_t<'x', 20, scalar<int>>>>::signature>);
	STATIC_REQUIRE(std::is_same_v<typename decltype(structure_renamed2)::signature, typename tuple_t<'s', array_t<'y', 10, array_t<'x', 20, scalar<int>>>, array_t<'y', 10, array_t<'x', 20, scalar<int>>>>::signature>);
...
TEST_CASE("Rename ignores overriden dimensions", "[rename]") {
	...
	STATIC_REQUIRE(std::is_same_v<typename decltype(structure_renamed)::signature, typename array_t<'z', 10, array_t<'w', 20, scalar<int>>>::signature>);
