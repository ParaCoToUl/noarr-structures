...
TEST_CASE("State shortcuts lit", "[state]") {
	...
	STATIC_REQUIRE(s1.get<noarr::length_in<'x'>>() == 10);
	STATIC_REQUIRE(s1.get<noarr::length_in<'y'>>() == 20);
	...
	STATIC_REQUIRE(s2.get<noarr::length_in<'x'>>() == 10);
	STATIC_REQUIRE(s2.get<noarr::length_in<'y'>>() == 20);
	STATIC_REQUIRE(s2.get<noarr::index_in<'x'>>() == 05);
	STATIC_REQUIRE(s2.get<noarr::index_in<'y'>>() == 15);

	const auto [s2x, s2y] = noarr::get_indices<'x', 'y'>(s2);

	STATIC_REQUIRE(noarr::get_index<'x'>(s2) == 05);
	STATIC_REQUIRE(noarr::get_index<'y'>(s2) == 15);
