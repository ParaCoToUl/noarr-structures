...
TEST_CASE("bcast test", "[has_test]") {
	...
	STATIC_REQUIRE(testee ^ set_length<'y'>(2) ^ fix<'x'>(4) ^ fix<'y'>(1) | has_offset());
	STATIC_REQUIRE((testee ^ set_length<'y'>(2) ^ fix<'x'>(4) ^ fix<'y'>(1) | offset()) == 1 * 42 * sizeof(int) + 4 * sizeof(int));
