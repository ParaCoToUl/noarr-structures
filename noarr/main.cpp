...
int main() {
    ...
    static_assert(std::is_pod<decltype(v)>::value, "a struct has to be a pod");
    static_assert(std::is_pod<decltype(v2)>::value, "a struct has to be a pod");
    static_assert(std::is_pod<decltype(t)>::value, "a struct has to be a pod");
    ...
    static_assert(std::is_pod<decltype(vs)>::value, "a struct has to be a pod");
    ...
    static_assert(std::is_pod<decltype(vs2)>::value, "a struct has to be a pod");
    ...
    static_assert(std::is_pod<decltype(vs2 % fixs<'y', 'x'>{5, 5})>::value, "a struct has to be a pod");
    std::cout << "vs2 % get_offset<'y'>{5}:" << (vs2 % get_offset<'y'>{5}) << std::endl << std::endl;

    auto vs3 = v2 % cresize<'x', 10>{}; // transform
    static_assert(std::is_pod<decltype(vs3)>::value, "a struct has to be a pod");
    ...
    static_assert(std::is_pod<decltype(vs4)>::value, "a struct has to be a pod");
    ...
    static_assert(std::is_pod<decltype(ts)>::value, "a struct has to be a pod");
}
