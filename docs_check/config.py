...
_NUL_FILE = "NUL" if sys.platform == "win32" else "/dev/null"
...
substitutions: dict[tuple[str, int], dict[str, str | Callable[[re.Match[str]], str]]] = {
    ('docs/BasicUsage.md', 1): {'auto': ''},
    ('docs/BasicUsage.md', 16): {'n(rows|cols)': '42'},
    ('docs/BasicUsage.md', 17): {_EPILOG: _tmp_mulsc + 'void UNIQ() { mul_by_scalar(sr, nullptr, 4.2f); mul_by_scalar(sc, nullptr, 4.2f); }\n'},
    ('docs/BasicUsage.md', 18): {_EPILOG: _tmp_mulsc + 'void UNIQ() { mul_by_scalar(br, 4.2f); mul_by_scalar(bc, 4.2f); }\n'},
    ('docs/BasicUsage.md', 19): {_EPILOG: _tmp_mulsc + 'void UNIQ() { mul_by_scalar(br, 4.2f); mul_by_scalar(bc, 4.2f); }\n'},
    ('docs/DefiningStructures.md', 5): {'image': 'UNIQ'},
    ('docs/DefiningStructures.md', 6): {r'using signature = \.\.\.;': '', r'\.\.\.': '0'},
    ('docs/DimensionKinds.md', 0): {_PROLOG: 'using noarr::lit;', '.*incorrect.*': ''},
    ('docs/DimensionKinds.md', 4): {'.*fails at compile time.*': ''},
    ('docs/DimensionKinds.md', 5): {'.*fails at compile time.*': '', '// option': '{//', r'// \.\.\.': '} } } }'},
    ('docs/Signature.md', 9): {r'/\*\.\.\.\*/::signature': _tmp_x, _EPILOG: f'static_assert(std::is_same_v<new_sig, {_tmp_y}>);\n', 'WANT_DEAD_CODE': '1'},
    ...
    ('docs/Traverser.md', 20): {'[abc]_data': '(void*)nullptr'},
    ('docs/Traverser.md', 21): {'[abc]_data': '(void*)nullptr'},
    ...
    ('docs/other/Mangling.md', 0): {r'/\*\.\.\.\*/': '0'},
    ('docs/other/SeparateLengths.md', 2): {'.*get_length.*': ''},
    ('docs/other/Serialization.md', 1): {r'/\*\.\.\.\*/': 'noarr::tuple_t<\'*\'>()', 'path/to/(src|dest)': _NUL_FILE, 'return 1': 'std::abort()'},
    ('docs/other/StructureTraits.md', 0): {'State = state<>': 'State = noarr::state<>', r'/\*\.\.\.\*/': 'void'},
    ('docs/structs/array.md', 0): {'using array = .*;': 'struct array_t;'},
    ('docs/structs/cuda_step.md', 1): {r'cuda_step_grid\(\)': 'step(0, 1024*1024)'},
    ('docs/structs/cuda_step.md', 2): {r'cuda_step_block\(\)': 'step(0, 1024*1024)'},
    ('docs/structs/cuda_step.md', 3): {r"cuda_step_block<'j'>\(\)": "step<'j'>(0, 1024*1024)"},
    ('docs/structs/cuda_step.md', 4): {r"cuda_step_grid<'t'>\(\)": "step<'t'>(0, 1024*1024)"},
